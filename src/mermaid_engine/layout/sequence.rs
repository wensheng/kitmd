...
pub(super) fn compute_sequence_layout(
    ...
    for idx in 0..=graph.edges.len() {
        if let Some(bucket) = notes_by_index.get(idx) {
            for note in bucket {
                ...
                if note.position == crate::mermaid_engine::ir::SequenceNotePosition::Over
    ...
    if !graph.sequence_frames.is_empty() && !message_ys.is_empty() {
        ...
        for frame in frames {
            ...
            let frame_label_text = match frame.kind {
                crate::mermaid_engine::ir::SequenceFrameKind::Alt => "alt",
                crate::mermaid_engine::ir::SequenceFrameKind::Opt => "opt",
                crate::mermaid_engine::ir::SequenceFrameKind::Loop => "loop",
                crate::mermaid_engine::ir::SequenceFrameKind::Par => "par",
                crate::mermaid_engine::ir::SequenceFrameKind::Rect => "rect",
                crate::mermaid_engine::ir::SequenceFrameKind::Critical => "critical",
                crate::mermaid_engine::ir::SequenceFrameKind::Break => "break",
            };
    ...
    for (_, _, event) in events {
        ...
        let stack = activation_stacks
            ...
            .or_default();
        match event.kind {
            crate::mermaid_engine::ir::SequenceActivationKind::Activate => {
                let depth = stack.len();
                stack.push((y, depth));
            }
            crate::mermaid_engine::ir::SequenceActivationKind::Deactivate => {
