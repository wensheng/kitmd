...
pub(super) fn compute_gitgraph_layout(
    ...
    if branches.is_empty() {
        branches.push(crate::mermaid_engine::ir::GitGraphBranch {
    ...
    let mut branch_entries: Vec<(crate::mermaid_engine::ir::GitGraphBranch, f32)> = branches
        .into_iter()
        .map(|branch| {
            let order = branch
                .order
    ...
    let mut commit_order: Vec<&crate::mermaid_engine::ir::GitGraphCommit> =
    ...
    for commit in commit_order {
        ...
        let show_label = gg.show_commit_label
            && commit.commit_type != crate::mermaid_engine::ir::GitGraphCommitType::CherryPick
            && (commit.commit_type != crate::mermaid_engine::ir::GitGraphCommitType::Merge
                || commit.custom_id);
        ...
        for parent in &commit.parents {
                ...
                (commit_pos.get(parent), commit_pos.get(&commit.id))
            {
                ...
                if let (Some(commit_a), Some(commit_b)) = (commit_a, commit_b) {
                    ...
                    if commit_b.commit_type == crate::mermaid_engine::ir::GitGraphCommitType::Merge
    ...
    for commit in &commit_layouts {
        let radius = if commit.commit_type == crate::mermaid_engine::ir::GitGraphCommitType::Merge {
...
fn commit_by_id<'a>(
    commits: &'a [crate::mermaid_engine::ir::GitGraphCommit],
    id: &str,
) -> Option<&'a crate::mermaid_engine::ir::GitGraphCommit> {
...
fn gitgraph_find_closest_parent_pos(
    commit: &crate::mermaid_engine::ir::GitGraphCommit,
    commit_pos: &HashMap<String, (f32, f32)>,
) -> Option<f32> {
    let closest_parent =
...
fn gitgraph_set_parallel_bt_pos(
    commits: &[&crate::mermaid_engine::ir::GitGraphCommit],
    default_pos: f32,
    commit_step: f32,
    layout_offset: f32,
    branch_pos: &HashMap<String, (f32, usize, f32, f32)>,
...
fn gitgraph_arrow_path(
    dir: Direction,
    commit_a: &crate::mermaid_engine::ir::GitGraphCommit,
    commit_b: &crate::mermaid_engine::ir::GitGraphCommit,
    p1: (f32, f32),
    p2: (f32, f32),
    commits: &[crate::mermaid_engine::ir::GitGraphCommit],
    config: &crate::mermaid_engine::config::GitGraphConfig,
    lanes: &mut Vec<f32>,
) -> String {
    let (p1x, p1y) = p1;
    let (p2x, p2y) = p2;
