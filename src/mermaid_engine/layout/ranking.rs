...
pub(super) fn rank_edges_for_manual_layout(
    ...
    if graph.kind != crate::mermaid_engine::ir::DiagramKind::Flowchart || layout_edges.len() < 3 {
        return layout_edges.to_vec();
    }

    let primary: Vec<crate::mermaid_engine::ir::Edge> = layout_edges
        .iter()
        .filter(|edge| edge.style != crate::mermaid_engine::ir::EdgeStyle::Dotted)
        .cloned()
        .collect();
    if primary.is_empty() {
        return layout_edges.to_vec();
...
pub(super) fn compute_ranks_subset(
    ...
    let subset_edges: Vec<crate::mermaid_engine::ir::Edge> = edges
        .iter()
...
mod tests {
    ...
    fn edge(from: &str, to: &str) -> crate::mermaid_engine::ir::Edge {
