...
fn is_region_subgraph(sub: &crate::mermaid_engine::ir::Subgraph) -> bool {
...
pub(in crate::mermaid_engine::layout) fn apply_flowchart_node_layout_cleanup(
    graph: &Graph,
...
pub(in crate::mermaid_engine::layout) fn debug_assert_flowchart_node_layout_invariants(
    graph: &Graph,
...
pub(in crate::mermaid_engine::layout) fn compress_linear_subgraphs(
    graph: &Graph,
    nodes: &mut BTreeMap<String, NodeLayout>,
    config: &LayoutConfig,
) {
...
pub(in crate::mermaid_engine::layout) fn enforce_top_level_subgraph_gap(
    graph: &Graph,
    nodes: &mut BTreeMap<String, NodeLayout>,
    theme: &Theme,
    config: &LayoutConfig,
) {
    if graph.kind != crate::mermaid_engine::ir::DiagramKind::Flowchart || graph.subgraphs.len() < 2
    {
        return;
    }
...
pub(in crate::mermaid_engine::layout) fn separate_sibling_subgraphs(
    graph: &Graph,
    nodes: &mut BTreeMap<String, NodeLayout>,
    theme: &Theme,
    config: &LayoutConfig,
...
pub(in crate::mermaid_engine::layout) fn align_disconnected_top_level_subgraphs(
    graph: &Graph,
    nodes: &mut BTreeMap<String, NodeLayout>,
) {
    if graph.kind != crate::mermaid_engine::ir::DiagramKind::Flowchart || graph.subgraphs.len() < 2
...
pub(in crate::mermaid_engine::layout) fn align_single_entry_top_level_subgraphs(
    graph: &Graph,
    nodes: &mut BTreeMap<String, NodeLayout>,
    config: &LayoutConfig,
) {
...
pub(in crate::mermaid_engine::layout) fn align_disconnected_components(
    graph: &Graph,
    nodes: &mut BTreeMap<String, NodeLayout>,
    config: &LayoutConfig,
) {
