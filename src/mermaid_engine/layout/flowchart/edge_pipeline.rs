...
fn choose_outer_back_edge_sides(
    from: &NodeLayout,
    to: &NodeLayout,
    direction: crate::mermaid_engine::ir::Direction,
...
pub(in crate::mermaid_engine::layout) struct RoutedEdgeBuildContext<'a> {
    pub(in crate::mermaid_engine::layout) graph: &'a Graph,
    pub(in crate::mermaid_engine::layout) nodes: &'a BTreeMap<String, NodeLayout>,
    pub(in crate::mermaid_engine::layout) subgraphs: &'a [SubgraphLayout],
    pub(in crate::mermaid_engine::layout) config: &'a LayoutConfig,
    pub(in crate::mermaid_engine::layout) layout_node_count: usize,
    pub(in crate::mermaid_engine::layout) edge_route_labels: &'a [Option<TextBlock>],
    pub(in crate::mermaid_engine::layout) edge_start_labels: &'a [Option<TextBlock>],
    pub(in crate::mermaid_engine::layout) edge_end_labels: &'a [Option<TextBlock>],
    pub(in crate::mermaid_engine::layout) label_dummy_ids: &'a [Option<String>],
    pub(in crate::mermaid_engine::layout) tiny_graph: bool,
    pub(in crate::mermaid_engine::layout) stage_metrics: Option<&'a mut LayoutStageMetrics>,
}

pub(in crate::mermaid_engine::layout) fn build_routed_edges(
    ctx: RoutedEdgeBuildContext<'_>,
) -> Vec<EdgeLayout> {
    ...
    for (_, _, _, idx) in &route_order {
        ...
        let start_inset = if edge.arrow_start {
            crate::mermaid_engine::edge_geometry::arrowhead_inset(graph.kind, edge.arrow_start_kind)
        } else {
            0.0
        };
        let end_inset = if edge.arrow_end {
            crate::mermaid_engine::edge_geometry::arrowhead_inset(graph.kind, edge.arrow_end_kind)
        } else {
            0.0
        };
...
mod tests {
    use super::*;
    use crate::mermaid_engine::ir::{NodeShape, NodeStyle};
