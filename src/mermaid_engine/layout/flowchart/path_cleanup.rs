...
pub(in crate::mermaid_engine::layout) fn reduce_orthogonal_path_crossings(
    graph: &Graph,
    nodes: &BTreeMap<String, NodeLayout>,
    routed_points: &mut [Vec<(f32, f32)>],
    config: &LayoutConfig,
    ...
    let use_perimeter_candidates = matches!(
        graph.kind,
        crate::mermaid_engine::ir::DiagramKind::Er | crate::mermaid_engine::ir::DiagramKind::State
    );
...
pub(in crate::mermaid_engine::layout) fn flowchart_path_hits_non_endpoint_nodes(
    path: &[(f32, f32)],
    from_id: &str,
    to_id: &str,
    nodes: &BTreeMap<String, NodeLayout>,
...
pub(in crate::mermaid_engine::layout) fn detour_flowchart_paths_around_non_endpoint_nodes(
    graph: &Graph,
    nodes: &BTreeMap<String, NodeLayout>,
    routed_points: &mut [Vec<(f32, f32)>],
    config: &LayoutConfig,
...
pub(in crate::mermaid_engine::layout) fn deoverlap_flowchart_paths(
    graph: &Graph,
    nodes: &BTreeMap<String, NodeLayout>,
    routed_points: &mut [Vec<(f32, f32)>],
    config: &LayoutConfig,
...
pub(in crate::mermaid_engine::layout) fn simplify_flowchart_axis_oscillations(
    routed_points: &mut [Vec<(f32, f32)>],
) {
...
pub(in crate::mermaid_engine::layout) fn simplify_flowchart_detour_rectangles(
    graph: &Graph,
    nodes: &BTreeMap<String, NodeLayout>,
    routed_points: &mut [Vec<(f32, f32)>],
) {
