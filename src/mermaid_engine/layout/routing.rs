...
impl RouteEndpoints {
    ...
    fn finish(self, ctx: &RouteContext<'_>, routed: Vec<(f32, f32)>) -> Vec<(f32, f32)> {
        ...
        crate::mermaid_engine::edge_geometry::apply_endpoint_insets(
...
pub(super) fn shape_polygon_points(node: &NodeLayout) -> Option<Vec<(f32, f32)>> {
    ...
    match node.shape {
        ...
        | crate::mermaid_engine::ir::NodeShape::Text => {
        ...
        | crate::mermaid_engine::ir::NodeShape::ParallelogramAlt => {
            let offset = w * 0.18;
            let points = if node.shape == crate::mermaid_engine::ir::NodeShape::Parallelogram {
                vec![
                    (x + offset, y),
                    (x + w, y),
                    (x + w - offset, y + h),
        ...
        | crate::mermaid_engine::ir::NodeShape::TrapezoidAlt => {
            let offset = w * 0.18;
            let points = if node.shape == crate::mermaid_engine::ir::NodeShape::Trapezoid {
                vec![
                    (x + offset, y),
                    (x + w - offset, y),
                    (x + w, y + h),
...
pub(super) fn route_edge_with_avoidance(
    ...
    if ctx.from_id == ctx.to_id {
        ...
        let Some(best_idx) = best_idx else {
            ...
            return crate::mermaid_engine::edge_geometry::apply_endpoint_insets(
...
pub(super) fn edge_pair_key(edge: &crate::mermaid_engine::ir::Edge) -> (String, String) {
