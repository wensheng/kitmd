...
use crate::mermaid_engine::ir::{DiagramKind, Direction, Graph};
...
pub(super) fn is_region_subgraph(sub: &crate::mermaid_engine::ir::Subgraph) -> bool {
...
fn subgraph_should_anchor(
    ...
    if graph.kind == crate::mermaid_engine::ir::DiagramKind::Flowchart
        || graph.kind == crate::mermaid_engine::ir::DiagramKind::State
    {
...
pub(super) fn subgraph_anchor_id<'a>(
    sub: &'a crate::mermaid_engine::ir::Subgraph,
    nodes: &BTreeMap<String, NodeLayout>,
) -> Option<&'a str> {
...
fn subgraph_layout_config(graph: &Graph, anchorable: bool, config: &LayoutConfig) -> LayoutConfig {
    ...
    if graph.kind == crate::mermaid_engine::ir::DiagramKind::Flowchart && anchorable {
...
fn flowchart_subgraph_internal_edge_count(
    graph: &Graph,
    sub: &crate::mermaid_engine::ir::Subgraph,
) -> usize {
...
pub(super) fn subgraph_padding_from_label(
    ...
    let internal_edge_count = if graph.kind == crate::mermaid_engine::ir::DiagramKind::Flowchart {
        flowchart_subgraph_internal_edge_count(graph, sub)
    } else {
        0
    };
    let has_internal_cycle = graph.kind == crate::mermaid_engine::ir::DiagramKind::Flowchart
        && internal_edge_count >= sub.nodes.len().max(1);

    let (mut pad_x, mut pad_y) = if graph.kind == crate::mermaid_engine::ir::DiagramKind::Flowchart
    {
        flowchart_subgraph_padding(graph.direction)
    } else if graph.kind == crate::mermaid_engine::ir::DiagramKind::Kanban {
        (KANBAN_SUBGRAPH_PAD, KANBAN_SUBGRAPH_PAD)
    } else {
        let base_padding = if graph.kind == crate::mermaid_engine::ir::DiagramKind::State {
    ...
    if graph.kind == crate::mermaid_engine::ir::DiagramKind::Flowchart && has_internal_cycle {
    ...
    let top_padding = if label_empty {
        pad_y
    } else if graph.kind == crate::mermaid_engine::ir::DiagramKind::Flowchart {
...
pub(super) fn resolve_subgraph_style(
    sub: &crate::mermaid_engine::ir::Subgraph,
    graph: &Graph,
) -> crate::mermaid_engine::ir::NodeStyle {
    ...
    if subgraphs.len() > 1 {
        ...
        for &i in &order {
            ...
            for &j in &all_descendants[i] {
                ...
                let pad = if graph.kind == crate::mermaid_engine::ir::DiagramKind::State {
...
mod tests {
    use super::*;
    use crate::mermaid_engine::ir::{Direction, Edge, EdgeStyle, Graph, NodeShape, Subgraph};
